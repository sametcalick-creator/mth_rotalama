        ...
        if df_base is None or df_base.empty:
            ...
            if filtreli_rapor.empty:
                ...
                if not b_filtreli_rapor.empty:
                    ...
                    for c in ["Toplam Kofra", "Toplam İş", "Uzun Öğle", "İlk İş Uzun", "Son-Önceki İş Uzun"]:
                ...
                if map_valid_df.empty:
                    ...
                    if map_valid_df.empty:
                        ...
                        popup_series = (
                            ...
                            "<b>Ekip / Kod:</b> " + map_valid_df["Ekip"].astype(str) + "<br>"
                            "<b>Personel (Ad Soyad):</b> " + p_name + "<br>"
                            "<b>Bölge:</b> " + map_valid_df["BÖLGE"].astype(str) + "<br>"
                            "<b>Tarih:</b> " + map_valid_df["Çizelgeleme Tarihi TR"].astype(str) + "<br>"
                            "<b>Saat:</b> " + map_valid_df["Çizelgeleme Saati"].astype(str) + "<br>"
                            "<b>Kofra / Hizmet No:</b> " + map_valid_df["Kofra"].astype(str) + "<br>"
                            "<b>Aktivite Tipi:</b> " + map_valid_df["Aktivite Tipi"].astype(str) + "<br>"
                            "<b>Adres:</b> " + map_valid_df["Adres"].astype(str) + "</div>"
                        )

                        tooltip_series = map_valid_df["Ekip"].astype(str) + " | " + p_name + " | Kofra: " + map_valid_df["Kofra"].astype(str)
                ...
                if secili_personel and not kart_df.empty:
                    ...
                    kacak_puan = kart_df.get("P_Kacak_Toplam", 0)
                    ...
                    with col_card2:
                        ...
                        with c2:
                            score_detail_card("🚨 Kaçak", f"{int(kacak_puan)} / 20", f"%{int((kacak_puan/20)*100)}")
                ...
                with col_dl1:
                    ...
                    with pd.ExcelWriter(excel_out, engine='openpyxl') as writer:
                        if not filtreli_rapor_filtreli.empty:
                            filtreli_rapor_filtreli.to_excel(writer, sheet_name='Personel Analizi', index=False)
                        if not b_filtreli_rapor.empty:
                            b_filtreli_rapor.to_excel(writer, sheet_name='Bölge Özeti', index=False)
