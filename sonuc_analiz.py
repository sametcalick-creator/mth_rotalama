...
def kvkk_sutun_kontrol(columns):
    ...
    veya kişisel isim içeren sütunlar (örn: Evin Adresi, Tesisat Adresi, Müşteri Adı, T.C. No)
    olup olmadığını kontrol eder. Sayac_Tipi_Adi, Hizmet_Nok_Tip_Adi vb. teknik alanlar engellenmez.
    """
    YASAKLI_ICERIK = ["adres", "vergi", "soyad", "tckn"]
    NON_PERSONAL_INDICATORS = ["sayac", "hizmet", "ag_og", "agog", "tarife", "aktivite", "ihbar", "islem", "birim", "kofra", "trafo", "bolge", "tesisat", "il_adi", "ilce_adi"]
    YASAKLI_NAME_PATTERNS = ["adi soyadi", "ad soyad", "adi_soyadi", "ad_soyad", "musteri ad", "personel ad", "abone ad", "kullanici ad", "mulk ad"]
    YASAKLI_EXACT_TOKENS = ["tc", "ad", "soyad", "adı", "adi"]
    ...
    for col in columns:
        ...
        # 1. Genel içerik kontrolü (adres, vergi, soyad, tckn)
        matched = False
        ...
        # Muafiyet Kontrolü: Sayaç Tipi Adı, Hizmet Nok Tip Adı, AG OG Adı vb. teknik nesne adları
        is_non_personal = False
        for indicator in NON_PERSONAL_INDICATORS:
            if indicator in col_lower:
                is_non_personal = True
                break
        if is_non_personal:
            continue

        # 2. Adı Soyadı / Ad Soyad tam kalıplar
        for pattern in YASAKLI_NAME_PATTERNS:
            if pattern in col_lower.replace("_", " "):
                if col_str not in bulunanlar:
                    bulunanlar.append(col_str)
                matched = True
                break
        ...
        # 3. Bağımsız kelime / token kontrolü (TC, AD, SOYAD, ADI)
        tokens_lower = col_no_dot.replace("_", " ").replace("-", " ").replace("/", " ").split()
        for w in YASAKLI_EXACT_TOKENS:
...
def compute_bolge_rapor(filtreli_rapor, df_filt=None):
    ...
    for col, agg in valid_agg.items():
        if col in bolge_satirlari.columns:
            if col in ["Uzun Öğle", "İlk İş Uzun", "Son-Önceki İş Uzun"]:
                mean_val = bolge_satirlari[col].mean()
                genel[col] = round(float(mean_val), 1) if pd.notna(mean_val) else 0
            elif agg == "mean":
...
# ======================================================
# EXCEL DIŞA AKTARIM (TEMİZ VE DÜZENLİ FORMAT)
# ======================================================

def export_sonuc_excel(filtreli_rapor, b_filtreli_rapor):
    """
    Sonuç Analizi Excel raporunu temiz, düzenli ve insan tarafından okunabilir sütunlarla hazırlar.
    Ham zaman saniyelerini (ilk_sn, ilk_kontak_sn vb.) kaldırıp yerine HH:MM formatlı saatleri yerleştirir.
    """
    output = BytesIO()

    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        # 1. Personel Analizi Sayfası
        if filtreli_rapor is not None and not filtreli_rapor.empty:
            p_df = filtreli_rapor.copy()

            # Formatlı saat sütunlarının varlığından emin ol
            if "İlk İş (Ort.)" not in p_df.columns and "ilk_sn" in p_df.columns:
                p_df["İlk İş (Ort.)"] = p_df["ilk_sn"].apply(saniye_formatla)
            if "Son İş (Ort.)" not in p_df.columns and "son_sn" in p_df.columns:
                p_df["Son İş (Ort.)"] = p_df["son_sn"].apply(saniye_formatla)
            if "İlk Kontak (Ort.)" not in p_df.columns and "ilk_kontak_sn" in p_df.columns:
                p_df["İlk Kontak (Ort.)"] = p_df["ilk_kontak_sn"].apply(saniye_to_saat)
            if "Son Kontak (Ort.)" not in p_df.columns and "son_kontak_sn" in p_df.columns:
                p_df["Son Kontak (Ort.)"] = p_df["son_kontak_sn"].apply(saniye_to_saat)

            # Sütun İsimlerini Temizle
            p_rename = {
                "Personel": "Personel Kodu",
                "PERSONEL Sicil No": "Sicil No",
                "ADI SOYADI": "Adı Soyadı",
                "BÖLGE": "Bölge",
                "Genel_Skor": "Genel Skor",
                "İlk Kontak (Ort.)": "İlk Kontak",
                "Son Kontak (Ort.)": "Son Kontak",
                "İlk İş (Ort.)": "İlk İş",
                "Son İş (Ort.)": "Son İş",
                "Ort. Süre (Dk)": "Ort. İş Süresi (Dk)",
                "Ort. İş Arası Süre": "Ort. İş Arası Süre (Dk)",
            }
            p_df = p_df.rename(columns=p_rename)

            # Mantıklı ve Temiz Sütun Sıralaması (Ham ham saniye sütunları hariç)
            p_target_cols = [
                "Personel Kodu", "Sicil No", "Adı Soyadı", "Bölge", "Genel Skor",
                "Gün Sayısı", "Toplam İş", "Toplam Kofra", "Ort. İş", "Ort. Kofra",
                "Kaçak Kontrol", "At, Sayaç Değiştirme", "Niteliksiz İşler",
                "Ort. Mesafe", "Toplam Mesafe",
                "İlk Kontak", "Son Kontak", "İlk İş", "Son İş",
                "Ort. İş Süresi (Dk)", "Ort. İş Arası Süre (Dk)",
                "Uzun Öğle", "İlk İş Uzun", "Son-Önceki İş Uzun", "Uzun İş Arası Süre"
            ]

            existing_cols = [col for col in p_target_cols if col in p_df.columns]
            p_excel = p_df[existing_cols].copy()

            for col in p_excel.columns:
                if col in ["Genel Skor", "Ort. İş", "Ort. Kofra", "Ort. Mesafe", "Toplam Mesafe", "Ort. İş Süresi (Dk)", "Ort. İş Arası Süre (Dk)"]:
                    p_excel[col] = pd.to_numeric(p_excel[col], errors="coerce").round(1)

            p_excel.to_excel(writer, sheet_name="Personel Analizi", index=False)

        # 2. Bölge Özeti Sayfası
        if b_filtreli_rapor is not None and not b_filtreli_rapor.empty:
            b_df = b_filtreli_rapor.copy()

            if "Ort. İlk İş" not in b_df.columns and "ilk_sn" in b_df.columns:
                b_df["Ort. İlk İş"] = b_df["ilk_sn"].apply(saniye_formatla)
            if "Ort. Son İş" not in b_df.columns and "son_sn" in b_df.columns:
                b_df["Ort. Son İş"] = b_df["son_sn"].apply(saniye_formatla)
            if "İlk Kontak (Ort.)" not in b_df.columns and "ilk_kontak_sn" in b_df.columns:
                b_df["İlk Kontak (Ort.)"] = b_df["ilk_kontak_sn"].apply(saniye_to_saat)
            if "Son Kontak (Ort.)" not in b_df.columns and "son_kontak_sn" in b_df.columns:
                b_df["Son Kontak (Ort.)"] = b_df["son_kontak_sn"].apply(saniye_to_saat)
            ...
            b_target_cols = [
                "Bölge", "Genel Skor", "Toplam İş", "Toplam Kofra", "Ort. İş", "Ort. Kofra",
                "Ort. Mesafe", "İlk Kontak", "Son Kontak", "İlk İş", "Son İş",
                "Ort. İş Süresi (Dk)", "Ort. İş Arası Süre (Dk)",
                "Uzun Öğle", "İlk İş Uzun", "Son-Önceki İş Uzun"
            ]

            existing_b_cols = [col for col in b_target_cols if col in b_df.columns]
            b_excel = b_df[existing_b_cols].copy()

            for col in b_excel.columns:
                if col in ["Genel Skor", "Ort. İş", "Ort. Kofra", "Ort. Mesafe", "Ort. İş Süresi (Dk)", "Ort. İş Arası Süre (Dk)"]:
                    b_excel[col] = pd.to_numeric(b_excel[col], errors="coerce").round(1)

            b_excel.to_excel(writer, sheet_name="Bölge Özeti", index=False)
