...
try:
    ...
    # Önce DejaVuSans fontunu kullan (Türkçe karakter desteği için)
    from reportlab.pdfbase.ttfonts import TTFont
    import matplotlib.font_manager as fm
    dejavu_path = fm.findfont("DejaVu Sans")
    pdfmetrics.registerFont(TTFont('DejaVuSans', dejavu_path))
    # Bold için
    dejavu_bold_path = fm.findfont("DejaVu Sans", fontext='ttf', weight='bold')
...
def generate_pdf_bulletin(
    ...
    for c in ["Toplam Kofra", "Toplam İş", "Kaçak Personel Adet", "Kaçak Personel Nitelikli Adet", "Kaçak Personel Niteliksiz Adet",
              "Toplam Kaçak Adet", "Toplam Nitelikli Adet", "Toplam Niteliksiz Adet", "Uzun Öğle", "İlk İş Uzun", "Son-Önceki İş Uzun"]:
        if c in b_disp_df.columns:
    ...
    if secilen_bolge == "Tümü":
        ...
        # 2. İlk İş ve İlk Kontak (Yan Yana 1 Sayfada)
        add_kpi_page(["İlk İş", "İlk Kontak"], side_by_side=True)
        
        # 3. Son İş ve Son Kontak (Yan Yana 1 Sayfada)
        add_kpi_page(["Son İş", "Son Kontak"], side_by_side=True)    
    if secilen_bolge == "Tümü":
